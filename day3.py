...
def priority(i):
    return i - ord('a') + 1 if i >= ord('a') else i - ord('A') + 27

def fn(line):
    c1 = set(line[:len(line)//2])
    c2 = set(line[len(line)//2:])
    ch = next(iter(c1.intersection(c2)))
    i = ord(ch)
    return priority(i)

# out = [fn(line) for line in test2.split('\n')]


def fn_chunk(group):
    badge = set(group[0]) & set(group[1]) & set(group[2])
    return priority(ord(badge.pop()))


chunked = chunk(test2.split('\n'), 3)
out = map(fn_chunk, chunked)
