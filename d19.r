...
solve := freeze \bp_line, minutes -> (
	...
	(
    	...
    	while (#switch (next)
        	(next == geode and ((res[ore] < geode_bot_ore_cost) or (res[obs] < geode_bot_obs_cost)))
        	or (next == obs and (res[ore] < obs_bot_ore_cost or res[clay] < obs_bot_clay_cost))
        	or (next == clay and res[ore] < clay_bot_cost)
        	or (next == ore and res[ore] < ore_bot_cost))
        (
